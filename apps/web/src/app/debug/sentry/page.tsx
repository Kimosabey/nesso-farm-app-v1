...
export default function SentryDebugPage() {
  ...
  function throwClient() {
    throw new Error(`Sentry web client throw — ${new Date().toISOString()}`);
  }

  function captureMessage() {
    const id = Sentry.captureMessage(`Sentry web message — ${new Date().toISOString()}`, 'info');
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 640, margin: '0 auto', color: '#e6f0ea' }}>
      <h1 style={{ fontSize: '1.5rem', marginBottom: '0.5rem' }}>Sentry debug · web</h1>
      <p style={{ opacity: 0.7, marginBottom: '1.5rem' }}>
        Each button fires one event. Check the <code>nesso-web</code> project in Sentry within ~30s.
      </p>
